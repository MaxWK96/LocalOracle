    ...
    <footer className="border-t border-border/50 bg-background mt-auto">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-10">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8">
          ...
          {/* Hackathon */}
          <div>
            <h3 className="text-sm font-semibold mb-3">Chainlink Convergence 2026</h3>
            <p className="text-sm text-muted-foreground mb-3">
              Submitted across multiple prize categories
            </p>
            <div className="flex flex-wrap gap-1.5 text-[10px]">
              {["Prediction Markets", "DeFi", "AI Agents", "Tokenization"].map((cat) => (
                <span key={cat} className="px-2 py-0.5 rounded bg-secondary text-muted-foreground border border-border/50">
                  {cat}
                </span>
              ))}
            </div>
        ...
        <div className="border-t border-border/50 pt-6 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-muted-foreground">
          ...
          <div className="flex items-center gap-4">
            ...
            <Link href="/agent" className="hover:text-accent transition-colors">
              Agent
            </Link>
          </div>
        </div>
