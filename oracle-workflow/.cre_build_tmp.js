...
var configSchema = exports_external.object({
  ...
  openWeatherApiKey: exports_external.string(),
  weatherApiKey: exports_external.string(),
  anthropicApiKey: exports_external.string()
});
var isOWMRain = (code2) => code2 >= 200 && code2 < 700;
var WEATHERAPI_RAIN_CODES = new Set([
  1063,
  1150,
  1153,
  1168,
...
var isWeatherAPIRain = (code2) => WEATHERAPI_RAIN_CODES.has(code2);
var scanMarkets = (runtime2, evmClient, contractAddress) => {
  runtime2.log("");
  runtime2.log("[1/5] Scanning on-chain markets...");
  const nextIdCallData = encodeFunctionData({
    abi: PredictionMarketABI,
    functionName: "nextMarketId"
  });
  const nextIdResult = evmClient.callContract(runtime2, {
    call: encodeCallMsg({
      from: zeroAddress,
      to: contractAddress,
      data: nextIdCallData
  ...
  if (!nextIdHex || nextIdHex === "0x") {
    runtime2.log("  Contract returned empty data — contract may not be deployed yet");
    runtime2.log("  No markets to process.");
    return [];
  }
  ...
  runtime2.log(`  Total markets: ${nextMarketId}`);
  const now = BigInt(Math.floor(Date.now() / 1000));
  const pending = [];
  for (let i2 = 0n;i2 < nextMarketId; i2++) {
    ...
    if (!marketHex || marketHex === "0x") {
      runtime2.log(`  Market #${i2}: empty response, skipping`);
      continue;
    }
    ...
    if (!market.resolved && market.endTime > 0n && market.endTime <= now) {
      pending.push(market);
      runtime2.log(`  Market #${market.id}: "${market.question}" — EXPIRED, needs settlement`);
    }
  }
  runtime2.log(`  Found ${pending.length} market(s) to settle`);
  return pending;
};
var fetchOpenWeatherMap = (sendRequester, lat, lng, apiKey) => {
  ...
  const code2 = data.weather?.[0]?.id ?? 800;
  const description = data.weather?.[0]?.description ?? "unknown";
  return {
    isRaining: isOWMRain(code2),
    description,
    source: "OpenWeatherMap",
    conditionCode: code2
  };
};
var fetchWeatherAPI = (sendRequester, lat, lng, apiKey) => {
  ...
  const data = json(response);
  const code2 = data.current?.condition?.code ?? 1000;
  const description = data.current?.condition?.text ?? "unknown";
  return {
    isRaining: isWeatherAPIRain(code2),
    description,
    source: "WeatherAPI",
    conditionCode: code2
  };
};
var adjudicateWithAI = (httpClient, runtime2, market, owm, wa, lat, lng) => {
  runtime2.log("[4/5] Sources disagree — calling Anthropic Claude to adjudicate...");
...
Respond with EXACTLY one word: YES or NO`;
  ...
  const fetchAI = httpClient.sendRequest(runtime2, (sendRequester, config) => {
  ...
  const aiOutcome = fetchAI(runtime2.config).result();
  runtime2.log(`  AI verdict: ${aiOutcome ? "YES (raining)" : "NO (not raining)"}`);
  return aiOutcome;
};
var settleMarket = (runtime2, evmClient, contractAddress, config, marketId, outcome) => {
  runtime2.log(`[5/5] Settling market #${marketId} on-chain — outcome: ${outcome ? "YES" : "NO"}`);
  const callData = encodeFunctionData({
    abi: PredictionMarketABI,
  ...
  if (resp.txStatus === TxStatus.SUCCESS) {
    ...
    runtime2.log(`  Settlement successful! TxHash: ${txHash}`);
  } else if (resp.txStatus === TxStatus.REVERTED) {
    runtime2.log(`  Settlement REVERTED: ${resp.errorMessage || "unknown reason"}`);
  } else {
    runtime2.log(`  Settlement FAILED: status=${resp.txStatus}, error=${resp.errorMessage || "none"}`);
  }
};
var settleMarkets = (runtime2) => {
  const config = runtime2.config;
  runtime2.log("================================================================");
  runtime2.log("  LocalOracle — Multi-Source Weather Oracle Settlement");
  runtime2.log("  Chainlink Convergence Hackathon 2026");
  runtime2.log("================================================================");
  ...
  const contractAddress = config.predictionMarketAddress;
  const pendingMarkets = scanMarkets(runtime2, evmClient, contractAddress);
  if (pendingMarkets.length === 0) {
    runtime2.log("");
    runtime2.log("No markets to settle. Workflow complete.");
    runtime2.log("================================================================");
    return "no-markets";
  }
  ...
  for (const market of pendingMarkets) {
    runtime2.log("");
    runtime2.log(`--- Processing Market #${market.id} ---`);
    runtime2.log(`  Question: "${market.question}"`);
    const lat = Number(market.lat) / 1e6;
    const lng = Number(market.lng) / 1e6;
    runtime2.log(`  Location: ${lat.toFixed(4)}, ${lng.toFixed(4)}`);
    runtime2.log("");
    runtime2.log("[2/5] Fetching weather data from 2 sources...");
    const owmFetcher = httpClient.sendRequest(runtime2, fetchOpenWeatherMap, ConsensusAggregationByFields({
      isRaining: identical,
    ...
    const waFetcher = httpClient.sendRequest(runtime2, fetchWeatherAPI, ConsensusAggregationByFields({
      isRaining: identical,
      description: identical,
      source: identical,
      conditionCode: identical
    ...
    runtime2.log(`  OpenWeatherMap: "${owmResult.description}" (code ${owmResult.conditionCode}) → rain=${owmResult.isRaining}`);
    runtime2.log(`  WeatherAPI:     "${waResult.description}" (code ${waResult.conditionCode}) → rain=${waResult.isRaining}`);
    runtime2.log("");
    runtime2.log("[3/5] Running consensus logic...");
    let outcome;
    let consensusMethod;
    if (owmResult.conditionCode === -1 && waResult.conditionCode === -1) {
      runtime2.log("  BOTH weather APIs failed. Skipping market — will retry next cycle.");
      continue;
    } else if (owmResult.conditionCode === -1) {
      outcome = waResult.isRaining;
      consensusMethod = "single-source (WeatherAPI, OWM failed)";
      runtime2.log(`  OpenWeatherMap failed. Using WeatherAPI: ${outcome ? "YES" : "NO"}`);
    } else if (waResult.conditionCode === -1) {
      outcome = owmResult.isRaining;
      consensusMethod = "single-source (OpenWeatherMap, WA failed)";
      runtime2.log(`  WeatherAPI failed. Using OpenWeatherMap: ${outcome ? "YES" : "NO"}`);
    } else if (owmResult.isRaining === waResult.isRaining) {
      outcome = owmResult.isRaining;
      consensusMethod = "unanimous (both sources agree)";
      runtime2.log(`  CONSENSUS: Both sources agree → ${outcome ? "YES (raining)" : "NO (not raining)"}`);
    } else {
      consensusMethod = "ai-adjudicated (sources disagreed)";
      outcome = adjudicateWithAI(httpClient, runtime2, market, owmResult, waResult, lat, lng);
    }
    runtime2.log("");
    settleMarket(runtime2, evmClient, contractAddress, config, market.id, outcome);
    settledCount++;
    runtime2.log("");
    runtime2.log(`  Market #${market.id} Summary:`);
    runtime2.log(`    Question:   "${market.question}"`);
    runtime2.log(`    OWM:        ${owmResult.description} (rain=${owmResult.isRaining})`);
    runtime2.log(`    WeatherAPI: ${waResult.description} (rain=${waResult.isRaining})`);
    runtime2.log(`    Method:     ${consensusMethod}`);
    runtime2.log(`    Outcome:    ${outcome ? "YES" : "NO"}`);
  }
  runtime2.log("");
  runtime2.log("================================================================");
  runtime2.log("  SETTLEMENT COMPLETE");
  runtime2.log(`  Markets scanned:  ${pendingMarkets.length}`);
  runtime2.log(`  Markets settled:  ${settledCount}`);
  runtime2.log(`  Markets skipped:  ${pendingMarkets.length - settledCount}`);
  runtime2.log("================================================================");
...
var onCronTrigger = (runtime2, payload) => {
  ...
  runtime2.log(`Cron triggered at: ${new Date().toISOString()}`);
  return settleMarkets(runtime2);
};
